...
func (s *server) handleListRooms(w http.ResponseWriter, r *http.Request) {
	...
	for _, e := range entries {
		var count int
		if e.cr == nil {
			if e.meta.System && e.meta.ReportedCount > 0 {
...
func main() {
	...
	for _, m := range []roomMeta{
		{Code: "TEST01", Name: "测试房间", HostName: "系统", LimitPlayers: true, MaxPlayers: 1, System: true, ReportedCount: 1, CreatedAt: time.Now()},
	} {
		m := m
