...
internal sealed class TournamentChatButtonController : MonoBehaviour
{
    private const string LogCategory = "TournamentChatButton";
    private const string CanvasObjectName = "JBS_TournamentChatButtonCanvas";
    private const string ButtonObjectName = "JBS_TournamentChatButton";
    ...
    {
        if (host.GetComponent<TournamentChatButtonController>() == null)
            host.AddComponent<TournamentChatButtonController>();
    }
    ...
    private void EnsureButton()
    {
        ...
        _buttonRect.anchoredPosition = new Vector2(-24f, -202f);
        ...
        JbsTmpFont.TryApply(_label, "聊天");

        JbsLog.Info(LogCategory, "Attached tournament chat button");
    }

    private void OnClicked()
    {
        if (TournamentRoomPanel.IsVisibleFrom(_buttonRect))
    ...
    private void RefreshVisual()
    {
        ...
        var open = TournamentRoomPanel.IsVisibleFrom(_buttonRect);
        _label.text = open ? "聊天 ON" : "聊天";
        _label.color = open
